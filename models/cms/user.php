<?php
	class cms_user_model extends model {
		...
		public function save_oke($user) {
			 ...
			 */
			if ($user["fullname"] == "") {
				$this->output->add_message("The full name cannot be empty.");
				$result = false;
			}
		...
		public function delete_user($user_id) {
			...
			array_push($queries,
				array("delete from passwords where container_id in (select id from containers where user_id=%d)", $user_id),
				array("delete from containers where user_id=%d", $user_id),
